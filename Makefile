...
# Local include directory for our header files
LOCAL_INCLUDE         ?= include/

# Local source directory for our source files
LOCAL_SOURCE          ?= src/
...
# Determines if ANSI formatting is supported
# boldOn = $(tput smso)
# boldOff = $(tput rmso)
# @echo $(blueOn) hello $(boldOff)
...
#------------------------------------------------------------------------------------------------
INCLUDE_PATHS = -I. -I$(RAYLIB_PATH)/src -I$(RAYLIB_PATH)/src/external -I$(RAYLIB_PATH)/src/extras -I$(LOCAL_INCLUDE)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ifeq ($(PLATFORM_OS),WINDOWS)
        ...
        # ORION DISABLED THE FOLLOWING LINES SO VSCODE TERMINAL ACTUALLY RECEIVES PRINTS!!!!!!
...
#------------------------------------------------------------------------------------------------
# PROJECT_SOURCE_FILES = $(wildcard $(BUILD_DIR)/*.o) # TODO: Make this work
...
$(PROJECT_NAME): $(OBJS)
	$(CC) -o $(PROJECT_NAME)$(EXT) $(addprefix $(BUILD_DIR), $(OBJS)) $(CFLAGS) $(INCLUDE_PATHS) $(LDFLAGS) $(LDLIBS) -D$(PLATFORM)
#	mv *.exe $(BUILD_DIR)

# Compile source files
# NOTE: This pattern will compile every module defined on $(OBJS)
%.o: $(LOCAL_SOURCE)%.c
	$(CC) -c $< -g -o $@ $(CFLAGS) $(INCLUDE_PATHS) -D$(PLATFORM)
	mv $@ $(BUILD_DIR)
...
clean_shell_sh:
    ifeq ($(PLATFORM),PLATFORM_DESKTOP)
        ...
        ifeq ($(PLATFORM_OS),OSX)
            find $(BUILD_DIR) -type f -perm +ugo+x -delete
            rm -f *.o
        endif
    endif
    ifeq ($(PLATFORM),PLATFORM_RPI)
